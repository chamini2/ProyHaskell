...
type Evolution = ( Int      -- Indicador de numero en Pokedx
...
type MonsterMove = ( Move   -- Move del Monster
                   , Int    -- PP restantes para este Move de este Monster
                   )
...
damage mov atk def = floor $ (*) modifier $ ((2 * aLVL + 10) / 250) * (aAtk / dDef) * mBase + 2
  ...
  modifier = let stab    = if elem (moveType mov) $ pokeType.species $ atk then 1.5 else 1
